...
public class TestChangePermitTile {
	...
	public void test() throws InsufficientResourcesException, InvalidRegionException {
		...
		deck.addAll(((GroupRegionalCity)(game.getGameMap().getGroupRegionalCity().get(0))).getPermissionDeckUp().getDeck());
		...
		for(Card card : deck) {
			assertFalse(((GroupRegionalCity)(game.getGameMap().getGroupRegionalCity().get(0))).getPermissionDeckUp().getDeck().contains(card));
		}
